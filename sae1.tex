...
plot[variable=\x,domain=0.001:3,samples=\gsamples,smooth,scale=\scale]
( {fx(\x)}, {fy(\x)} ) -- (0,0);
\draw[thick,gray!80!brown] (0,0) circle (\scale*0.5cm);
...
\filldraw[top color=white,bottom color=blue,fill opacity=.5] (0.5*\scale,0) circle (2pt) node[above right,scale=.9] {$1$};
\filldraw[top color=white,bottom color=blue,fill opacity=.5] (-0.5*\scale,0) circle (2.5pt) node[above right] {$-1$};
...
\draw[thick,left color=green,fill opacity=.5,right color=gray!80!brown] (-70:1) circle (2.5pt);
...
\draw (3,0) node[transfer] (h1) {$\displaystyle \frac{5}{s(s+2)}$};
\draw (0,0) node[circle,draw,minimum size=5mm] (mix) {};
\draw (h1.south) ++(-1,-1) node[transfer] (h2) {$\displaystyle \frac{4}{s+5}$};
...
\begin{enumgreekparen}
    ...
    \begin{itemize}
    	\item \textbf{Για} \( z=10 \):\todo{Graph 153}
...
\newpage
\appendix
\tcblistof[\section*]{defn}{Ορισμοί}
\tcblistof[\section*]{thm}{Θεωρήματα}
